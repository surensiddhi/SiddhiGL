...
import { listVouchers, approveVoucher, unapproveVoucher, postMany } from '../services/voucher';
...
export function init(el: HTMLElement): void {
  ...
  function renderTable(): void {
    ...
    // Checkbox column only on Approved tab (for posting)
    const showCheck = canPost && activeTab === 'approved';
  ...
  function rowHtml(r: VoucherRow, showCheck: boolean): string {
    ...
    if (canApprove && isUnapproved) actions.push(`<button class="btn btn-primary"   data-action="approve"   data-id="${r.id}" style="padding:3px 8px;font-size:0.78rem">Approve</button>`);
    ...
    if (canPost    && isApproved)   actions.push(`<button class="btn btn-primary"   data-action="post"      data-id="${r.id}" style="padding:3px 8px;font-size:0.78rem">Post</button>`);
  ...
  async function handleAction(btn: HTMLButtonElement): Promise<void> {
    ...
    try {
      if (action === 'approve') {
        ...
        const ok = await confirmDialog('Post this voucher? This cannot be undone.', 'Post Voucher');
        if (!ok) { btn.disabled = false; return; }
        await postMany([id]);
      }
      await load();
